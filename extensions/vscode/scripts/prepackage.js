...
if (!vscode.workspace.fs.stat(vscode.Uri.file(guiDist)).then(() => true)) {
  vscode.workspace.fs.createDirectory(vscode.Uri.file(guiDist));
}
...
// Put back index.html
if (vscode.workspace.fs.stat(vscode.Uri.file(indexHtmlPath))) {
  ...
  try {
    ...
    if (target.startsWith("linux")) {
      ...
      filesToRemove.forEach((file) => {
        ...
        if (
          vscode.workspace.fs.stat(vscode.Uri.file(filepath)).then(() => true)
        ) {
...
// tree-sitter-wasm
vscode.workspace.fs.createDirectory(vscode.Uri.file("out"));
...
for (const f of filesToCopy) {
  vscode.workspace.fs.copy(
    vscode.Uri.file(path.join(__dirname, f)),
    vscode.Uri.file(path.join(__dirname, "..", "out", path.basename(f))),
  );
...
if (ghAction() && isArm()) {
  ...
  vscode.workspace.fs.createDirectory(vscode.Uri.file("node_modules/@esbuild"));
  execCmdSync(
  ...
  vscode.workspace.fs.delete(
    vscode.Uri.file("node_modules/@esbuild/esbuild.zip"),
  );
} else {
...
vscode.workspace.fs.copy(
  vscode.Uri.file("node_modules/jsdom/lib/jsdom/living/xhr/xhr-sync-worker.js"),
