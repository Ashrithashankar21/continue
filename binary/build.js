...
vscode.workspace.fs.mkdirSync(bin);
vscode.workspace.fs.mkdirSync(out);
...
(async () => {
  ...
  vscode.workspace.fs.mkdirSync("bin/node_modules", { recursive: true });
  ...
  vscode.workspace.fs.mkdirSync(treeSitterWasmsDir);
  await new Promise((resolve, reject) => {
    ncp(
      path.join(
        __dirname,
  ...
  for (const assetPath of assetBackups) {
    vscode.workspace.fs.rmSync(assetPath, { force: true });
  }
  ...
  for (const target of targets) {
    ...
    vscode.workspace.fs.mkdirSync(targetDir, { recursive: true });
    ...
    vscode.workspace.fs.unlinkSync(`${targetDir}/build.tar.gz`);
